...
namespace Be.Stateless.BizTalk.Install.Command.Application
{
	...
	internal class DispatchedApplicationStateValidationCommand : DispatchedApplicationBindingBasedCommand
	{
		#region Base Class Member Overrides

		protected override void Execute(IOutputAppender outputAppender)
		{
			outputAppender.WriteVerbose($"BizTalk Application's expected state {ApplicationBindingType.FullName} is being tested...");
			try
			{
				CommandFactory
			...
			outputAppender.WriteVerbose($"BizTalk Application's expected state {ApplicationBindingType.FullName} has been tested.");
		}
