...
<#
.SYNOPSIS
   Deploys the resources declared by a given Microsoft BizTalk Server® library deployment manifest.
.DESCRIPTION
   This command will entrust Invoke-Build to carry on all the tasks necessary to deploy the resources declared by a given Microsoft BizTalk Server® library deployment
...
.EXAMPLE
   PS> Install-BizTalkLibrary -Manifest $m -TargetEnvironment DEV
.EXAMPLE
   PS> Install-BizTalkLibrary -Manifest $m -TargetEnvironment DEV -ExcludeResourceGroup * -Verbose

   As all the resources are excluded, this command outputs the tasks that would have carried on.
.EXAMPLE
   PS> Install-BizTalkLibrary -Manifest $m -TargetEnvironment DEV -Task { task Deploy { Write-Host 'I hijacked the whole deployment process.' } }
.LINK
